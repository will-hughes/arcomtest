...
for(my $i = 0; $i < $total; $i += $batch_size) {
    ...
    $updated_count += process_batch(\@batch_ids, $index_terms, $facets, $domains, $i + scalar(@batch_ids), $total);
}
...
sub process_batch {
    my ($batch_ids, $index_terms, $facets, $domains, $current, $total) = @_;
    ...
    foreach my $eprint_id (@$batch_ids) {
        ...
        # REPLACE existing subjects/divisions with taxonomy terms
        if( keys %found_index_terms ) {
            ...
            # Clear subjects/divisions if no taxonomy terms found
            $eprint->set_value( 'subjects', [] );
            $eprint->set_value( 'divisions', [] );
            $eprint->commit();
        }
    }
